...
int InitRenderDevice()
{
    ...
    Engine.screenBuffer2x = SDL_CreateTexture(Engine.renderer, SDL_PIXELFORMAT_RGB565, SDL_TEXTUREACCESS_STREAMING, SCREEN_XSIZE, SCREEN_YSIZE);
    if (!Engine.screenBuffer2x) {
        // PrintLog("FATAL ERROR: Failed to create gameRenderTexture: %s", SDL_GetError());
        SDL_DestroyRenderer(Engine.renderer); Engine.renderer = NULL;
        SDL_DestroyWindow(Engine.window); Engine.window = NULL;
...
#if !RETRO_USE_ORIGINAL_CODE
    float dimAmount = 1.0;
#if RETRO_PLATFORM != RETRO_PS3 //switch doesn't need this it's builtin
